...
def test_hrp_eval_transform_matches_official_non_antialiased_resize() -> None:
    ...
    assert resize.antialias is False
    output = transform(torch.zeros((3, 480, 640), dtype=torch.uint8))
    assert output.shape == (3, 224, 224)
    assert torch.isfinite(output).all()


def test_hrp_training_transform_matches_official_medium() -> None:
    ...
    blur = next(
        item
        for item in transform.transforms
        if isinstance(item, transforms.RandomApply)
    )
    assert blur.p == 1.0
